...
def draw_maze(stdscr, maze_obj):
    ...
    for y in range(maze_obj.height):
        for x in range(maze_obj.width):
            ...
            # We skip EAST and SOUTH checks because 
            # the neighbor's WEST and NORTH will draw them (hopefully lmao). 
            # (Except for the borders, but the corners should handle the structure).
